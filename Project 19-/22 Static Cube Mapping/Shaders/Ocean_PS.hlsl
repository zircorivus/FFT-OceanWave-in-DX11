...
float4 PS(VSOutput input) : SV_TARGET
{
    ...
    // Fresnel with roughness correction 菲涅尔效应：掠射角反射更多光
    float eta = 1.33f;
    ...
    // Geometry masking Smith 几何遮蔽：微表面互相遮挡，减少反射光
    ...
    float3 envReflect = g_Skybox.Sample(g_Sampler, reflectDir).rgb * _EnvironmentLightStrength; // 从天空盒采样反射颜色
    ...
    // ============================================================
    // AI-GENERATED: 边缘泡沫（Intersection Foam）
    // 匹配 Unity FFTOcean_Shader.shader frag() 中 intersect 计算
    // 原理：比较场景深度（球体）与水面深度，差值越小说明物体越贴近水面 → 泡沫越多
    // ============================================================
    float sceneNdcDepth = g_SceneDepthMap.Sample(g_Sampler, input.screenUV).r;  // 场景 NDC 深度 [0,1]
    float waterNdcDepth = input.ndcDepth;                                        // 水面 NDC 深度 [0,1]
    float depthDiff     = sceneNdcDepth - waterNdcDepth;  // >0 表示场景物体在水面后方
    float intersect     = 0.0f;
    if (depthDiff > 0.0f)  // 物体在水面之后（正常情况）
    {
        // smoothstep: depthDiff 越小 → 返回值越小 → intersect 越大 → 泡沫越多
        // 阈值 0.001 ≈ 1m@1000m远平面（NDC 深度空间近似，匹配 Unity _ProjectionParams.w = 1/farPlane）
        intersect = 1.0f - smoothstep(0.0f, 0.001f, depthDiff);
    }
    foam = foam + intersect * pow(foam, _EdgeFoamPower);  // 边缘泡沫加成
    // ============================================================
