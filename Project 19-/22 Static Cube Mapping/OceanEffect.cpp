...
public:
    ...
    ComPtr<ID3D11ShaderResourceView> m_pSceneDepthSRV; // AI-GENERATED: 场景深度纹理 SRV（用于边缘泡沫）
...
bool OceanEffect::InitAll(ID3D11Device* device)
{
    ...
    // AI-GENERATED: 创建几何着色器（重心坐标生成 + 数据透传）
    if (SUCCEEDED(D3DReadFileToBlob(L"Shaders\\Ocean_GS.cso", blob.GetAddressOf())))
        device->CreateGeometryShader(blob->GetBufferPointer(), blob->GetBufferSize(),
            nullptr, pImpl->m_pGeometryShader.GetAddressOf());
...
// AI-GENERATED: 场景深度纹理 setter（用于边缘泡沫深度比较）
void OceanEffect::SetSceneDepthMap(ID3D11ShaderResourceView* tex)
{
    pImpl->m_pSceneDepthSRV = tex;
}
...
void OceanEffect::Apply(ID3D11DeviceContext* deviceContext)
{
	...
	// 7. 曲面细分：设置 HS/DS/GS 着色器（拓扑由 SetRenderDefault → GetInputData 控制）
	if (pImpl->m_TessellationEnabled)
	{
		...
		deviceContext->GSSetShader(pImpl->m_pGeometryShader.Get(), nullptr, 0);  // AI-GENERATED: GS 绑定
	}
	else
	{
		deviceContext->HSSetShader(nullptr, nullptr, 0);
		deviceContext->DSSetShader(nullptr, nullptr, 0);
		deviceContext->GSSetShader(nullptr, nullptr, 0);  // AI-GENERATED: GS 解绑
	}
    ...
    // AI-GENERATED: PS 侧绑定场景深度纹理（t10，用于边缘泡沫深度比较）
    deviceContext->PSSetShaderResources(10, 1, pImpl->m_pSceneDepthSRV.GetAddressOf());
